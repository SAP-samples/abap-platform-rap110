...
**********************************************************************
CLASS lsc_zrap110_r_traveltp_SOL DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_zrap110_r_traveltp_SOL IMPLEMENTATION.
  ...
  METHOD adjust_numbers.
    ...
    IF mapped-travel IS NOT INITIAL.
      TRY.
          ...
          cl_numberrange_runtime=>number_get(
            EXPORTING
              nr_range_nr       = '01'
              object            = 'ZRAP110SOL'   "Fallback: '/DMO/TRV_M'
    ...
    IF mapped-booking IS NOT INITIAL.
      ...
      LOOP AT mapped-booking INTO DATA(mapped_booking) GROUP BY mapped_booking-TravelID.
        SELECT MAX( booking_id ) FROM zrap110_abookSOL WHERE travel_id = @mapped_booking-TravelID INTO @DATA(max_booking_id) .
 ...
 METHOD save_modified.
   ...
   IF update IS NOT INITIAL.
     ...
     "raise event
     RAISE ENTITY EVENT ZRAP110_R_TravelTP_SOL~travel_accepted
      FROM VALUE #(
     ...
     "raise event
     RAISE ENTITY EVENT ZRAP110_R_TravelTP_SOL~travel_rejected
      FROM VALUE #(
        FOR travel IN update-travel
        WHERE ( %control-OverallStatus EQ if_abap_behv=>mk-on AND
...
**************************************************************************
  METHOD setInitialTravelValues.
    ...
    DATA: update TYPE TABLE FOR UPDATE zrap110_r_traveltp_SOL\\Travel.
